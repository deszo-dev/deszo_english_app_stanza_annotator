...
def _resolve_config(args: argparse.Namespace) -> StanzaAnnotatorConfig:
    log_level = args.log_level or os.getenv("STANZA_ANNOTATOR_LOG_LEVEL", "info")
    logging_config = LoggingConfig.model_validate(
        {
            "enabled": _env_bool("STANZA_ANNOTATOR_LOGGING", default=False),
            "level": log_level,
        }
    )
    return StanzaAnnotatorConfig.model_validate(
        {
            ...
            "debug": args.debug or _env_bool("STANZA_ANNOTATOR_DEBUG", default=False),
